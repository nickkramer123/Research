...
for i, (b_key, arr) in enumerate(values_pkl.items()):
    ...
    lyap = nolds.lyap_r(values_array[:, 0], emb_dim=3)
    chaos_measures.append(lyap)  # Store it
...
# do the same for chaos measures
with open('choas_scores.txt', 'w') as file:
    for chaos in chaos_measures:
        file.write(str(chaos) + '\n')
